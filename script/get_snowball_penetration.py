"""Test script: Run Steps 1-5 for Snowball holdings.

Steps:
  1. Parse snowball PDF
  4. Classify records
...
Usage:
  ...
  python -m script.get_snowball_penetration                        # shallow mode (fast)
  python -m script.get_snowball_penetration --deep --file snowball_20260227.pdf               # deep mode (full stock-level)
  python -m script.get_snowball_penetration --date 20251231 --file snowball_20260227.pdf      # specify holdings date
  python -m script.get_snowball_penetration --deep --date 20251231 --file snowball_20260227.pdf # deep + date
"""
...
ap = argparse.ArgumentParser(description="Test snowball fund penetration")
...
if __name__ == "__main__":
    ...
    # Step 1: Parse snowball only
    from parsers.snowball_parser import SnowballParser

    parser = SnowballParser()
    records = parser.parse(f"data/{file_name}")
    logger.info(f"Step 1 done: {len(records)} records parsed from snowball")

    # Load config
    with open("config.yaml", "r") as f:
        config = yaml.safe_load(f)

    # Use snowball-specific cache subdirectory
    config["paths"]["cache_dir"] = "cache/snowball"

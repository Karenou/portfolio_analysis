"""Test script: Run Steps 1-5 for Alipay holdings.
...
Usage:
  ...
  python -m test.test_alipay_penetration                        # shallow mode (fast)
  python -m test.test_alipay_penetration --deep --file alipay_20260225.pdf               # deep mode (full stock-level)
  python -m test.test_alipay_penetration --date 20251231 --file alipay_20260225.pdf      # specify holdings date
  python -m test.test_alipay_penetration --deep --date 20251231 --file alipay_20260225.pdf # deep + date
"""
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Test alipay fund penetration")
    ap.add_argument("--deep", action="store_true", help="Enable deep penetration mode")
    ap.add_argument("--date", type=str, default="",
                    help="Holdings date in YYYYMMDD format, e.g. 20251231")
    ap.add_argument("--file", type=str, help="file name")
    args = ap.parse_args()
    deep_mode = args.deep
    date_str = args.date
    file_name = args.file

    # Step 1: Parse alipay only
    from parsers.alipay_parser import AlipayParser

    parser = AlipayParser()
    records = parser.parse(f"data/{file_name}")
    logger.info(f"Step 1 done: {len(records)} records parsed from alipay")

    # Load config
    with open("config.yaml", "r") as f:
        config = yaml.safe_load(f)

    # Use alipay-specific cache subdirectory
    config["paths"]["cache_dir"] = "cache/alipay"
